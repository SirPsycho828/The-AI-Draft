import { useConfig } from '../../hooks/useConfig';
import { ModelPicker } from '../../components/admin/ModelPicker';

export default function AdminSettings() {
  const { config, loading } = useConfig();

  if (loading) return <p className="text-muted-foreground">Loading config...</p>;

  if (!config) {
    return (
      <div>
        <h1 className="font-heading text-2xl tracking-[0.03em] text-foreground mb-6">SETTINGS</h1>
        <p className="text-muted-foreground">
          No config document found. Initialize the config by refreshing models with your OpenRouter API key.
        </p>
      </div>
    );
  }

  return (
    <div>
      <h1 className="font-heading text-2xl tracking-[0.03em] text-foreground mb-6">SETTINGS</h1>
      <div className="space-y-8">
        <ModelPicker config={config} />
      </div>
    </div>
  );
}
